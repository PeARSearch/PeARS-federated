...
def filter_instances_by_language():
    ...
    for i in instances:
        ...
        if i.rstrip("/") == current_app.config["SITENAME"].rstrip("/"):
            logging.warning(f"WARNING: It seems like you're trying to federate with yourself. Consider removing the name of your local site from .known_hosts.txt if it's on it. For now, I'm skipping this instance ({i}).")
        ...
        try:
            ...
            logging.error(f"\t>> ERROR: filter_instances_by_language: request failed trying to access {url}; error message {e}")
            skipped_instances.append({"instance": i, "reason": "connection error for /api/languages"})
            continue
        if resp.status_code != 200:
            logging.error(f"\t>> ERROR: filter_instances_by_language: got non-200 status code when trying to access {url}...")    
        ...
        try:
            ...
            continue
        if resp.status_code != 200:
            logging.error(f"\t>> ERROR: filter_instances_by_language: got an error code trying to access {url}...")
        ...
        try:
            ...
            logging.error(f"\t>> ERROR: filter_instances_by_language: request failed trying to access {identity_info_url}, error message: {e}")
            identity_info = {
                "url": i,
...
def get_best_instances(query, lang, instances, m, top_k=3):
    ...
    logging.info(f"BEST INSTANCES {[i['url'] for i in best_instances]}")
...
def get_cross_instance_results(query, instances):
    ...
    for instance in best_instances:
        ...
        try:
            ...
            logging.info(f"Request to remote instance (url={url}) took {t_delta:.3f}s")
        except Exception as e:
            logging.error(f"Error when connecting to {url}, error message: {e}")
        ...
        if req_success and resp.status_code == 200:
            ...
            logging.error(f"Got non-200 status code from {url}")
            remote_results = {}
